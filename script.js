// ---------- day/night palette tự động theo giờ ----------
  ...
  const ALBUM_IMAGE = ""; // để trống hoặc URL hình ảnh album
  ...
  // Previous/Next (loop vì chỉ có 1 file)
  ...
  // ---------- lấy số confession hôm nay từ Apps Script ----------
  function fetchTodayCount(){
    ...
    fetch(SCRIPT_URL)
      ...
      .then(data => {
        if (data.success) {
          todayCounter.textContent = "☀ " + data.todayCount + " người đã chia sẻ hôm nay";
          todayCounter.classList.add('show');
        }
      })
      .catch(() => { /* im lặng bỏ qua nếu lỗi */ });
  }
  fetchTodayCount();
